export const appShellClass =
  "min-h-screen bg-gradient-to-br from-brand-100 via-brand-50 to-brand-200 px-4 py-4 md:px-7 md:py-7";

export const appCenterClass =
  "mx-auto grid min-h-[calc(100vh-2rem)] w-full place-items-center md:min-h-[calc(100vh-3.5rem)]";

export const panelClass =
  "w-full max-w-[860px] rounded-card border border-brand-200/80 bg-white/95 p-5 shadow-card backdrop-blur-sm md:p-6";

export const pageTitleClass = "mb-2 text-display text-ink-900";

export const subtitleClass = "text-sm text-ink-600";

export const helperTextClass = "text-xs text-ink-500";

export const formStackClass = "mb-4 grid gap-2.5";

export const fieldLabelClass = "grid gap-1.5 text-sm font-medium text-ink-700";

export const inputClass =
  "w-full rounded-control border border-brand-200 bg-white px-3 py-2.5 text-body text-ink-800 shadow-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100 disabled:cursor-not-allowed disabled:opacity-70";

export const textareaClass = `${inputClass} min-h-24 resize-y`;

export const primaryButtonClass =
  "w-full rounded-control bg-brand-500 px-3 py-2.5 text-sm font-semibold text-white transition hover:bg-brand-600 disabled:cursor-not-allowed disabled:opacity-70";

export const ghostButtonClass =
  "rounded-control border border-brand-200 bg-brand-50 px-3 py-2 text-sm font-medium text-brand-600 transition hover:bg-brand-100 disabled:cursor-not-allowed disabled:opacity-70";

export const dangerButtonClass =
  "rounded-control bg-rose-600 px-3 py-2 text-sm font-medium text-white transition hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-70";

export const errorTextClass = "my-2 text-sm font-semibold text-rose-700";
